...
public class TickHandlerTGS implements ITickHandler
{
	...
	void sendPacket(int x, int y, int z, World worldObj, Player player)
	{
	...
	void updatePlayerPos(EntityPlayer player)
	{
		posX = player.posX;
		posZ = player.posZ;
	}

	int[] getNearestSapling(World world, int xpos, int ypos, int zpos)
	{
		for (int x = -5; x <= 5; x++)
			for (int y = -2; y <= 2; y++)
				for (int z = -5; z <= 5; z++)
